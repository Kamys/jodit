...
const JODIT_TABLE_PROCESSOR_BINDED = '__jodit_tableprocessor_binded';
 ...
 * @param {Jodit} editor
 */
export function tableProcessor(editor: Jodit) {
    editor.events.on('change afterInit', debounce(() => {
        if (editor.editor) {
            $$('table', editor.editor).forEach((elm: HTMLElement) => {
                if (!(<any>elm)[JODIT_TABLE_PROCESSOR_BINDED]) {
                    (<any>elm)[JODIT_TABLE_PROCESSOR_BINDED] = true;
